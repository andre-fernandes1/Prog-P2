...
def add_data():
    ...
    # Guarda período anterior (para limpeza opcional se quiser manter)
    ...
    with st.form("form_adicionar_obra"):
        ...
        # (não removemos keys aqui — a leitura segura será feita no submit)
        ...
        if submitted:
            # Leitura segura dos campos diretamente do session_state
            ...
            # pega a matéria a partir das keys possíveis (ordem importa: p1, p2, other)
            materia_val = None
            if st.session_state.get("add_materia_p1", None):
                ...
                # fallback: tenta usar a variável local ' _ ' (pouco provável)
                materia_val = ""
            ...
            if not nome_val or not autor_val or not materia_val:
                ...
                # Inserção usando periodo_val (evita inconsistências)
                if periodo_val == '1º Período':
                    if materia_val == 'Teoria do Direito':
                        ...
                        branch = "teoria_do_estado_democratico"
                        ...
                        branch = "pensamento_juridico_brasileiro"
